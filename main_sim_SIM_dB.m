...
sicDbSet = zeros(MrisLen,mrMtLen,MdSetLen);
nMonte = 50;
for kk = 1 : MrisLen
    MrisSize = MrisSet(kk,:);
            ...
            for nn = 1 : nMonte
                if nn == 1 || mod(nn, 10) == 0
                    fprintf(['\n',num2str(MrisSize(1)),'x',num2str(MrisSize(2)),' RIS, ', num2str(MrSize(1)),'x',num2str(MrSize(2)),...
                        ' Array, ', 'Md = ',num2str(Md),', nn = ',num2str(nn),', ',datestr(now,"HH:MM"), '\n']);
                end
...
for kk = 1 : MrisLen
    for ii = 1 : mrMtLen
        plot(MdSet,squeeze(sicDbSet(kk,ii,:)),plt{kk,ii},'linewidth',2,'markersize',8)
        hold on
    end
end
grid on
